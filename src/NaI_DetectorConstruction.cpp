...
void  NaI_DetectorConstruction::BuildSiCDDetector(G4Material* material, G4LogicalVolume* mothervol,
    ...
    for (int p = 0; p < 16; ++p) {
        ...
        //for (int k = 0; k < 4; ++k) {
            ...
            new G4PVPlacement(rotZ,
                              ...
                              mothervol,
                              false,
                              copyOffset + p +64,  // continue copy number from front
                              true);
...
BuildSiCDDetector(Si, logicWorld,
                  7.0 * cm,
                  (7.+0.00325+0.000001)*cm,           // z-position
